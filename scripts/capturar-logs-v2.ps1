...
if (!(Test-Path $ADB)) {
    ...
    if (!(Test-Path $ADB)) {
        Write-Host "ADB nao encontrado em: $ADB" -ForegroundColor Red
        Write-Host "Certifique-se de que o Android SDK esta instalado" -ForegroundColor Red
        exit 1
    }
}
...
Write-Host "  CAPTURANDO LOGS DE DIAGNOSTICO V14" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "  - RegistrarTrocaPanoUseCase (inicio, mesa lookup, criacao, insercao)" -ForegroundColor White
Write-Host "  - SettlementViewModel.registrarTrocaPanoNoHistorico (chamada, parametros)" -ForegroundColor White
Write-Host "  - MesasReformadasViewModel.carregarMesasReformadas (dados, cards gerados)" -ForegroundColor White
Write-Host "  - Erros e excecoes gerais" -ForegroundColor White
Write-Host ""
...
# Capturar logs com filtros especificos
$cmd = "& `"$ADB`" logcat -v time DEBUG_CARDS:* *:E"
...
Write-Host "Salve os logs em um arquivo para analise:" -ForegroundColor Cyan
