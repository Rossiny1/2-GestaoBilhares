# ========================================
# ANALISADOR DE LOGS DE CRASH - GESTAO BILHARES
...
if ([string]::IsNullOrEmpty($LogFile)) {
    ...
    if ($recentLogs) {
        ...
        Write-Host "❌ Nenhum arquivo de log encontrado. Execute primeiro: .\capturar-logs-crash-otimizado.ps1" -ForegroundColor Red
        exit 1
    }
}
...
if (-not (Test-Path $LogFile)) {
    Write-Host "❌ Arquivo não encontrado: $LogFile" -ForegroundColor Red
    exit 1
}
...
Write-Host "1. ESTATÍSTICAS GERAIS:" -ForegroundColor Cyan
...
if ($stackTraces) {
    ...
    Write-Host "ℹ️ Nenhum stack trace do app encontrado" -ForegroundColor Gray
}
...
if ($dbErrors) {
    Write-Host "🗄️ ERROS DE BANCO DE DADOS:" -ForegroundColor Red
...
if ($authErrors) {
    Write-Host "🔐 ERROS DE AUTENTICAÇÃO:" -ForegroundColor Red
    $authErrors | ForEach-Object { 
        Write-Host "   $($_.Line)" -ForegroundColor Red 
    }
} else {
    Write-Host "✅ Nenhum erro de autenticação encontrado" -ForegroundColor Green
}
...
if ($memoryErrors) {
    Write-Host "💾 ERROS DE MEMÓRIA/PERFORMANCE:" -ForegroundColor Red
    $memoryErrors | ForEach-Object { 
        Write-Host "   $($_.Line)" -ForegroundColor Red 
    }
} else {
    Write-Host "✅ Nenhum erro de memória/performance encontrado" -ForegroundColor Green
}
...
if ($permissionErrors) {
    Write-Host "🔒 ERROS DE PERMISSÃO:" -ForegroundColor Red
    $permissionErrors | ForEach-Object { 
        Write-Host "   $($_.Line)" -ForegroundColor Red 
    }
} else {
    Write-Host "✅ Nenhum erro de permissão encontrado" -ForegroundColor Green
}

Write-Host ""

# Resumo e recomendações
Write-Host "10. RESUMO E RECOMENDAÇÕES:" -ForegroundColor Cyan
...
if ($totalErrors -eq 0) {
    ...
    Write-Host "🚨 CRASH CRÍTICO DETECTADO - App não está funcionando" -ForegroundColor Red
    ...
    Write-Host "   - Corrija o código baseado no erro" -ForegroundColor White
} elseif ($errorCount -gt 0) {
    Write-Host "⚠️ ERROS DETECTADOS - App pode ter problemas" -ForegroundColor Yellow
    Write-Host "   - Verifique os erros específicos acima" -ForegroundColor White
    Write-Host "   - Corrija os problemas identificados" -ForegroundColor White
} else {
    Write-Host "ℹ️ APENAS WARNINGS/INFO - App funcionando com avisos" -ForegroundColor Blue
}
