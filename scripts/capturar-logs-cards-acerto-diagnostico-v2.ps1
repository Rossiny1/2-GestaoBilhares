...
# Versão: 2.0 - Corrigido com sintaxe PowerShell correta
...
# Construir argumentos como array para evitar problemas com PowerShell
$arguments = @("logcat", "-v", "time", "-s", 
    "DEBUG_CARDS:*", 
    "RegistrarTrocaPanoUseCase:*", 
    "SettlementViewModel:*", 
    "NovaReformaViewModel:*", 
    "MesasReformadasViewModel:*", 
    "AppRepository:*", 
    "BaseViewModel:*", 
    "AndroidRuntime:*")

# Executar comando com argumentos separados (sintaxe PowerShell correta)
& $ADB $arguments | ForEach-Object {
    $line = $_
