/**
 * Script simples para executar migração de claims
 * Usa Firebase Admin SDK já instalado no projeto
 */
...
try {
    ...
    if (error.code === 'app/already-initialized') {
        ...
        console.error('');
        console.error('Certifique-se de estar autenticado no Firebase CLI:');
        console.error('  firebase login');
        console.error('');
        console.error('Ou configure GOOGLE_APPLICATION_CREDENTIALS com service account key');
        process.exit(1);
    }
}

/**
 * Simular chamada da função migrateUserClaims
 * (Reimplementa a lógica diretamente aqui)
 */
        ...
        for (const empresaDoc of empresasSnapshot.docs) {
            ...
            for (const colaboradorDoc of colaboradoresSnapshot.docs) {
                ...
                try {
                    ...
                    const rotasSnapshot = await colaboradorRotasRef
                    ...
                    results.success++;
                    console.log(`   ✅ ${email} (${rotasIds.length} rotas)`);
                } catch (error) {
                    results.failed++;
