...
for ($i = 0; $i -lt $totalLines; $i++) {
    ...
    $clienteJson = $cliente | ConvertTo-Json -Depth 10
    $clienteJson | Out-File -FilePath $tempFile -Encoding UTF8
    
    try {
        # Importar via Firebase CLI (usando comando de criação de documento)
        $result = & $firebaseCmd firestore:delete clientes/$id --confirm 2>$null
        $result = & $firebaseCmd firestore:create clientes/$id --data "$clienteJson" 2>$null
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            $erros++
            Write-Host "[$($i+1)/$totalLines] ❌ Erro ao importar: $nome" -ForegroundColor Red
        }
    }
    catch {
        $erros++
