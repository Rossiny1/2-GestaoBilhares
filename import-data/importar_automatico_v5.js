/**
 * Importação Automática com Service Account Existente - V5
 * 
 * Usa a chave que você já tem
 * Importa todos os clientes automaticamente
 * ATUALIZAÇÃO: Valores corrigidos + Capitalização padronizada (versão limpa)
 */
...
/**
 * Formata texto com capitalização padrão (Primeira letra maiúscula, conectivos minúsculos)
 */
function formatarTextoPadronizado(texto) {
    if (!texto) return texto;

    const conectivos = new Set(['da', 'de', 'do', 'dos', 'das', 'e', 'dos', 'das']);
...
function mapearLinhaParaCliente(linha, rotaId, clienteId) {
    ...
    const nome = formatarTextoPadronizado(nomeBruto);
    const cpfCnpj = normalizarTexto(campos[2] ? campos[2].replace(/"/g, '').trim() : '');
    const endereco = formatarTextoPadronizado(campos[3] ? campos[3].replace(/"/g, '').trim() : '');
    const cidade = formatarTextoPadronizado(campos[4] ? campos[4].replace(/"/g, '').trim() : '');
    const estado = formatarTextoPadronizado(campos[5] ? campos[5].replace(/"/g, '').trim() : '');
    ...
    const observacoes = formatarTextoPadronizado(campos[12] ? campos[12].replace(/"/g, '').trim() : '');
    ...
    return {
        ...
        email: null,
        endereco: endereco,
        bairro: null, // Campo bairro não existe no CSV
        cidade: cidade,
        estado: estado,
        cep: null,
        latitude: null,
 ...
 */
async function main() {
    console.log('🚀 IMPORTAÇÃO AUTOMÁTICA - FIREBASE ADMIN SDK V5');
    ...
    const arquivosParaRotas = [
        {
            ...
            rota: '037-Salinas',
            descricao: 'Rota Salinas - Importação CSV (Valores corrigidos + Capitalização)'
        }
        // Adicione os outros arquivos aqui quando tiver:
        // { arquivo: '../anexos/Cadastro Clientes- 033-Montes Claros.csv', rota: '033-Montes Claros', descricao: 'Rota Montes Claros - Importação CSV (Valores corrigidos + Capitalização)' },
        // { arquivo: '../anexos/Cadastro Clientes- 08-Chapada Gaucha.csv', rota: '08-Chapada Gaucha', descricao: 'Rota Chapada Gaucha - Importação CSV (Valores corrigidos + Capitalização)' },
        // { arquivo: '../anexos/Cadastro Clientes- 035-Coração de Jesus.csv', rota: '035-Coração de Jesus', descricao: 'Rota Coração de Jesus - Importação CSV (Valores corrigidos + Capitalização)' },
        // { arquivo: '../anexos/Cadastro Clientes- 034-Bonito de Minas.csv', rota: '034-Bonito de Minas', descricao: 'Rota Bonito de Minas - Importação CSV (Valores corrigidos + Capitalização)' },
        // { arquivo: '../anexos/Cadastro Clientes- 03-Januária.csv', rota: '03-Januária', descricao: 'Rota Januária - Importação CSV (Valores corrigidos + Capitalização)' },
        // { arquivo: '../anexos/Cadastro Clientes- 036-Bahia.csv', rota: '036-Bahia', descricao: 'Rota Bahia - Importação CSV (Valores corrigidos + Capitalização)' }
    ];
    ...
    try {
        ...
        console.log(`   💰 Valores finais esperados: Ficha R$ 1,50 | Comissão R$ 0,60`);
        console.log(`   📝 Capitalização: José Alves Da Silva (padronizado)`);
        ...
        console.log('4. Verifique se os valores de ficha (R$ 1,50) e comissão (R$ 0,60) estão corretos');
        console.log('5. Confirme a capitalização padronizada nos nomes e endereços');
