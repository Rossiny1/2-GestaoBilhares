            ...
            for (document in snapshot.documents) {
                try {
                    ...
                    val fotoAntesLocal = if (!fotoAntesUrlFirebase.isNullOrBlank()) {
                        try {
                            val isUrlFirebase = com.example.gestaobilhares.core.utils.FirebaseStorageManager.isFirebaseStorageUrl(fotoAntesUrlFirebase)
                            if (isUrlFirebase) {
                    ...
                    val fotoDepoisLocal = if (!fotoDepoisUrlFirebase.isNullOrBlank()) {
                        try {
                            val isUrlFirebase = com.example.gestaobilhares.core.utils.FirebaseStorageManager.isFirebaseStorageUrl(fotoDepoisUrlFirebase)
                            if (isUrlFirebase) {
