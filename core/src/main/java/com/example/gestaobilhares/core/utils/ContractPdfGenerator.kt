...
data class ContractEmissionMetadata(
    ...
    val operadorEmail: String? = null,
    val operadorId: Long? = null,
    val operadorNivelAcesso: String? = null,
    val dispositivoModelo: String? = null,
...
class ContractPdfGenerator(
    ...
    fun generateContractPdf(
        ...
        // ✅ NOVO: Adicionar handler de rodapé para todas as páginas
        val timestampGeracao = SimpleDateFormat("dd/MM/yyyy HH:mm:ss", Locale("pt", "BR")).format(Date())
        val footerHandler = ContractFooterHandler(timestampGeracao)
        pdfDocument.addEventHandler(PdfDocumentEvent.END_PAGE, footerHandler)
        
        val document = Document(pdfDocument).apply {
            // Aumenta a margem inferior para evitar sobreposição com o rodapé
            setMargins(36f, 36f, 80f, 36f)
        }
        ...
        try {
            ...
            addDistratoSignatures(document, contrato, font, fontBold, representanteLegal)
            
            // ✅ NOVO: Registro de representação digital e metadados
            addDigitalRepresentationSection(document, font, fontBold, representanteLegal, emissionMetadata)
    ...
    fun generateDistratoPdf(
        ...
        try {
            ...
            // ✅ NOVO: Registrar bloco de representação digital também no distrato
            addDigitalRepresentationSection(document, font, fontBold, representanteLegal, emissionMetadata)
        } finally {
            document.close()
        }
    ...
    private fun addDistratoSignatures(
        document: Document,
        contrato: ContratoLocacao,
        ...
        val assinaturaLocador = representanteLegal?.assinaturaBase64 ?: contrato.assinaturaLocador
    ...
    private fun addDigitalRepresentationSection(
        document: Document,
        font: com.itextpdf.kernel.font.PdfFont,
        fontBold: com.itextpdf.kernel.font.PdfFont,
        representanteLegal: AssinaturaRepresentanteLegal?,
        emissionMetadata: ContractEmissionMetadata?
    ) {
        val metadata = resolveEmissionMetadata(emissionMetadata)
        
        val title = Paragraph("REGISTRO DE REPRESENTAÇÃO DIGITAL")
            .setFont(fontBold)
            .setFontSize(11f)
            .setMarginTop(20f)
            .setMarginBottom(6f)
        ...
        val representativeInfo = representanteLegal?.let {
            "Assinatura institucional da LOCADORA: ${it.nomeRepresentante} (${it.cargoRepresentante}), Procuração nº ${it.numeroProcuração}."
        } ?: "Assinatura institucional da LOCADORA registrada no sistema GestaoBilhares."
        document.add(
            Paragraph(representativeInfo)
                .setFont(font)
                .setFontSize(9f)
                .setMarginBottom(4f)
        )
        
        val operadorLine = buildString {
            append("Operador responsável: ${metadata.operadorNome}")
            metadata.operadorNivelAcesso?.let { append(" • Nível: $it") }
            metadata.operadorEmail?.takeIf { it.isNotBlank() }?.let { append(" • E-mail: $it") }
            metadata.operadorId?.let { append(" • ID interno: #$it") }
        }
        document.add(
            Paragraph(operadorLine)
                .setFont(font)
                .setFontSize(9f)
                .setMarginBottom(3f)
        )
        ...
        val deviceLine = buildString {
            append("Dispositivo: ${metadata.dispositivoModelo ?: "Não identificado"}")
            metadata.dispositivoId?.let { append(" • ID: $it") }
            metadata.appVersion?.let { append(" • App v$it") }
        }
        document.add(
        ...
        val timestampText = SimpleDateFormat("dd/MM/yyyy HH:mm:ss", Locale("pt", "BR"))
            .format(metadata.timestampOperacao)
        val compliance = Paragraph(
            "Registro eletrônico conforme MP 2.200-2/2001 e Lei 14.063/2020. Emissão: $timestampText."
        )
            .setFont(font)
            .setFontSize(9f)
            .setMarginBottom(10f)
    ...
    private fun resolveEmissionMetadata(emissionMetadata: ContractEmissionMetadata?): ContractEmissionMetadata {
        if (emissionMetadata != null) return emissionMetadata
        
        val session = UserSessionManager.getInstance(context)
        val operadorNome = session.getCurrentUserName().ifBlank { "Operador autorizado" }
        val operadorEmail = session.getCurrentUserEmail().ifBlank { null }
        val operadorId = session.getCurrentUserId().takeIf { it != 0L }
        val operadorNivel = session.getCurrentUserLevel().name
        val dispositivoId = runCatching {
        ...
        return ContractEmissionMetadata(
            operadorNome = operadorNome,
            operadorEmail = operadorEmail,
            operadorId = operadorId,
            operadorNivelAcesso = operadorNivel,
    ...
    /**
     * ✅ NOVO: Handler de eventos para adicionar rodapé em cada página
     * Adiciona timestamp de geração, número da página e total de páginas
     * Conforme recomendação de melhoria de segurança documental
     */
    private class ContractFooterHandler(
        private val timestampGeracao: String
    ) : IEventHandler {
        ...
        override fun handleEvent(event: Event) {
            ...
            // Criar canvas para desenhar no rodapé
            val pageSize = page.pageSize
            val canvas = PdfCanvas(page.newContentStreamBefore(), page.resources, pdfDocument)
            
            // Posição do rodapé (30 pontos do fundo)
            val footerY = 30f
            val footerRectangle = Rectangle(
                pageSize.left + 36f, // Margem esquerda
                footerY,
                pageSize.width - 72f, // Largura menos margens
                20f // Altura do rodapé
            )
            
            // Criar canvas de layout para adicionar texto formatado
            @Suppress("DEPRECATION")
            val layoutCanvas = Canvas(canvas, pdfDocument, footerRectangle)
            
            try {
                val font = PdfFontFactory.createFont()
                
                // Texto do rodapé: timestamp | Página X de Y
                val footerText = "Gerado em: $timestampGeracao | Página $pageNumber de $totalPages"
                ...
                layoutCanvas.add(footerParagraph)
                layoutCanvas.close()
                
            } catch (e: Exception) {
                android.util.Log.e("ContractPdfGenerator", "Erro ao adicionar rodapé: ${e.message}", e)
                // Em caso de erro, apenas fechar o canvas sem adicionar conteúdo
                try {
                    layoutCanvas.close()
                } catch (e2: Exception) {
