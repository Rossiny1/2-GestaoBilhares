...
**Fluxo Principal (híbrido View + Compose):** Login → Rotas → Clientes da Rota → Detalhes do Cliente → Acerto → Impressão
...
- Menu principal: Disponível para `ADMIN` aprovado e para `rossinys@gmail.com` (super admin). Lógica centralizada em `UserSessionManager`.
...
- Estratégia conservadora: substituir gradualmente Fragments por equivalentes Compose.
- Telas Compose reutilizam o design existente (cores, espaçamentos, hierarquia).
- Fragments podem atuar como wrappers (via `ComposeView`) até que a navegação seja 100% Compose.
