        ...
        fun bind(meta: MetaColaborador) {
            ...
            when {
                progresso >= 100 -> {
                    ...
                    binding.tvStatusChip.setTextColor(binding.root.context.getColor(com.example.gestaobilhares.ui.R.color.white))
                    binding.tvStatusChip.setBackgroundResource(com.example.gestaobilhares.ui.R.drawable.bg_success_chip)
                }
                progresso >= 80 -> {
                    binding.tvStatusChip.text = "PRÓXIMA"
                    binding.tvStatusChip.setTextColor(binding.root.context.getColor(com.example.gestaobilhares.ui.R.color.white))
                    binding.tvStatusChip.setBackgroundResource(com.example.gestaobilhares.ui.R.drawable.bg_warning_chip)
                }
                else -> {
                    binding.tvStatusChip.text = "EM ANDAMENTO"
                    binding.tvStatusChip.setTextColor(binding.root.context.getColor(com.example.gestaobilhares.ui.R.color.text_primary))
                    binding.tvStatusChip.setBackgroundResource(com.example.gestaobilhares.ui.R.drawable.bg_status_chip)
                }
            }
        }
    }
