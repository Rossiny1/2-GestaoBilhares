...
import com.example.gestaobilhares.ui.databinding.ItemCycleHistoryBinding
            ...
            binding.apply {
                ...
                when (ciclo.status) {
                    com.example.gestaobilhares.data.entities.StatusCicloAcerto.EM_ANDAMENTO -> {
                        ...
                        tvStatus.setTextColor(root.context.getColor(com.example.gestaobilhares.ui.R.color.white))
                        tvStatus.background.setTint(root.context.getColor(com.example.gestaobilhares.ui.R.color.orange_600))
                        tvStatus.visibility = View.VISIBLE
                    }
                    com.example.gestaobilhares.data.entities.StatusCicloAcerto.FINALIZADO -> {
                        tvStatus.text = "Finalizado"
                        tvStatus.setTextColor(root.context.getColor(com.example.gestaobilhares.ui.R.color.white))
                        tvStatus.background.setTint(root.context.getColor(com.example.gestaobilhares.ui.R.color.green_600))
                        tvStatus.visibility = View.VISIBLE
                    }
                    com.example.gestaobilhares.data.entities.StatusCicloAcerto.CANCELADO -> {
                        tvStatus.text = "Cancelado"
                        tvStatus.setTextColor(root.context.getColor(com.example.gestaobilhares.ui.R.color.white))
                        tvStatus.background.setTint(root.context.getColor(com.example.gestaobilhares.ui.R.color.red_600))
                        tvStatus.visibility = View.VISIBLE
                    }
                    com.example.gestaobilhares.data.entities.StatusCicloAcerto.PLANEJADO -> {
                        tvStatus.text = "Planejado"
                        tvStatus.setTextColor(root.context.getColor(com.example.gestaobilhares.ui.R.color.white))
                        tvStatus.background.setTint(root.context.getColor(com.example.gestaobilhares.ui.R.color.blue_600))
                        tvStatus.visibility = View.VISIBLE
                    }
                }
                // Cor do lucro
                val profitColor = if (ciclo.lucroLiquido >= 0) {
                    com.example.gestaobilhares.ui.R.color.green_600
                } else {
                    com.example.gestaobilhares.ui.R.color.red_600
                }
