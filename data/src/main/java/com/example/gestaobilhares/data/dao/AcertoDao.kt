...
interface AcertoDao {
    ...
    @Query("SELECT * FROM acertos WHERE cliente_id = :clienteId ORDER BY data_acerto DESC LIMIT :limit")
    fun buscarRecentesPorCliente(clienteId: Long, limit: Int): Flow<List<Acerto>>

    @Query("DELETE FROM acertos WHERE cliente_id = :clienteId AND id NOT IN (SELECT id FROM acertos WHERE cliente_id = :clienteId ORDER BY data_acerto DESC LIMIT :limit)")
