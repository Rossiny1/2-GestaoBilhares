# ========================================
# CAPTURADOR DE LOGS - CRASH MANUTENÇÃO MESA
# ========================================
# Script específico para capturar crash ao clicar em "Manutenção de Mesa"

Write-Host "========================================" -ForegroundColor Red
Write-Host "CAPTURADOR DE LOGS - CRASH MANUTENÇÃO MESA" -ForegroundColor Red
Write-Host "========================================" -ForegroundColor Red
Write-Host ""
...
Write-Host "4. Clique no botão FAB (botão +) na parte inferior" -ForegroundColor White
Write-Host "5. Clique em 'Manutenção Mesa'" -ForegroundColor White
Write-Host "6. O crash deve ocorrer - os logs serão capturados automaticamente" -ForegroundColor Yellow
Write-Host "7. Pressione Ctrl+C para parar a captura" -ForegroundColor Yellow
Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
...
$logFile = "logcat-crash-manutencao-mesa-$timestamp.txt"

# Tags para filtrar (focando no crash)
$tags = @(
    "LOG_CRASH",
    "RoutesFragment",
    "AndroidRuntime",
...
function Write-ColoredLog {
    ...
    # Detectar exceções
    if ($line -match "Exception|RuntimeException|IllegalStateException|NullPointerException|ClassCastException") {
        if ($line -match "LOG_CRASH.*Exception") {
    ...
    if ($line -match "ERROR|Error|Erro|Failed|Failure") {
        if ($line -match "LOG_CRASH.*ERRO|LOG_CRASH.*Erro") {
...
if (Test-Path $logFile) {
    ...
    if ($logContent) {
        ...
        if ($crashLogs -gt 0) {
            ...
            Write-Host "   4. Procure por logs 'LOG_CRASH' para ver o fluxo" -ForegroundColor White
            Write-Host ""
        }
    }
}
